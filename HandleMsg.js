...
module.exports = HandleMsg = async (aruga, message) => {
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 7) {
                        found === true 
                        aruga.reply(from, '*[ANTI STICKER SPAM]*\nnVocê tem SPAM STICKER no grupo, você será automaticamente chutado pelo bot', message.id).then(() => {
                            ...
                            if (found !== false) {
                                ...
                                const result = '✅ DB Sticker resetado'
        ...
        if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwner){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    aruga.reply(from, '*[GROUP LINK DETECTOR]*\nVocê enviou um link de bate-papo em grupo, desculpe por ter sido expulso do grupo:(', id).then(() => {
	...
	// Filter Banned People
        ...
        case 'help':
            ...
            break
        case 'menuadmin':
            if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado em grupos!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
        ...
        case 'join':
            ...
            if (!islink) return aruga.reply(from, 'Desculpe, o link do grupo está errado! por favor envie o link correto', id)
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await aruga.sendText(from, 'Grupo ingressado com sucesso via link!')
                ...
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await aruga.reply(from, 'Grupo ingressado com sucesso via link!', id)
                      })
                      .catch(() => {
                          aruga.reply(from, 'falha!', id)
                      })
            }
            break
        case 'botstat': {
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'falha em converter sticker em imagem!', id)
                .then(() => {
                    console.log(`Adesivo para imagem processada para ${processTime(t, moment())} Seconds`)
                })
        } else if (!quotedMsg) return aruga.reply(from, `Formato errado, marque o adesivo que você deseja transformar em imagem`, id)
        break
			
			
        // Sticker Creator
    case 'logopornhub':
            if (args.length === 1) return aruga.reply(from, `Enviar o pedido *#logopornhub [ |Teks1|Teks2 ]*,\n\n contoh : *#pornhub |Dimas| HUB*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                aruga.reply(from, `processo ....`, id)
	...
	case 'coolteks':
	case 'cooltext':
            if (args.length == 0) return aruga.reply(from, `Para criar um texto legal CoolText em uma imagem, use ${prefix}cooltext teks\n\nContoh: ${prefix}cooltext arugaz`, id)
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? aruga.sendText(from, 'Desculpe, o link que você enviou não contém imagens.')
            ...
            case 'antistiker':
                    ...
                    if (args[0] == 'on') {
                        ...
                        if(cek){
                            return aruga.reply(from, '*Anti Spam Sticker Detector* já está ativo neste grupo', id) //if number already exists on database
                        } else {
                            antisticker.push(chatId)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            aruga.reply(from, '*[Anti Sticker SPAM]* foi ativado \ nCada membro do grupo com mais de 7 adesivos de spam serão chutados pelo bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        ...
                        aruga.reply(from, `pilih on / off\n\n*[Anti Sticker SPAM]*\nCada membro do grupo que tiver um adesivo de spam será chutado pelo bot!`, id)
                    }
                    break
                    case 'antilink':
                    if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado em grupos!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo:)', id)
                    if (args[0] == 'on') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            return aruga.reply(from, '*Anti Group Link Detector* já está ativo neste grupo', id) //if number already exists on database
                        } else {
                            antilink.push(chatId)
                            fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
                            aruga.reply(from, '*[Anti Group Link]* foi ativado \ nCada membro do grupo que enviar uma mensagem contendo o link do grupo será expulso pelo bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(!cek){
	...
	case 'brainly':
        ...
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '[WAIT]em processo ⏳ silahkan tunggu ± 1 min!', id)
        ...
        case 'stickergiphy':
            ...
            if (isGiphy) {
                ...
                if (!gifUrl) { return aruga.reply(from, 'Falha ao buscar código giphy', id) }
                ...
                .catch(() => {
                    aruga.reply(from, `houve uma falha!`, id)
                })
            } else {
                await aruga.reply(from, 'Desculpe, o adesivo do comando giphy só pode usar links do giphy.  [Giphy Only]', id)
            }
            break
      case 'qrread':
    ...
    case 'qrcode':
      ...
      break			
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    ...
                    .catch(() => {
                        aruga.reply(from, 'houve um error!')
                    })
            } else {
                await aruga.reply(from, `Sem imagens! Silahkan kirim gambar dengan caption ${prefix}meme <teks_atas> | <teks_bawah>\ncontoh: ${prefix}meme teks atas | teks bawah`, id)
            }
            break
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                try {
                    ...
                    aruga.reply('Yahh o processo falhou, o conteúdo está correto ainda?..', id)
                }
            } else {
        ...
        case 'nulis':
            ...
            .catch(() => {
                aruga.reply(from, 'houve um error!', id)
            })
            break
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        //Media
        case 'ytmp3':
            if (args.length == 0) return aruga.reply(from, `Para baixar uma música do youtube\ntipo: ${prefix}ytmp3 [link_yt]`, id)
            ...
            .then(async(res) => {
				...
				.catch(() => {
					aruga.reply(from, `URL Ini ${args[0]} Já foi baixado antes. O URL será reiniciado após 1 hora / 60 minutos`, id)
				})
			})
            break
        case 'ytmp4':
		...
		case 'facebook':
			...
			.then(async (res) => {
				...
				if (res.status == 'error') return aruga.sendFileFromUrl(from, link, '', '', id)
				await aruga.sendFileFromUrl(from, linkhd, '', '', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', '', id)
					.catch(() => 
						aruga.reply(from, '', id)
					})
				})
		...
		//Primbon Menu
		case 'cekzodiak':
            if (args.length !== 4) return aruga.reply(from, `Para verificar o zodíaco, use ${prefix}cekzodiak nama tanggallahir bulanlahir tahunlahir\nContoh: ${prefix}cekzodiak fikri 13 06 2004`, id)
            ...
            break
		case 'artinama':
			if (args.length == 0) return aruga.reply(from, `Para saber o significado do nome de uma pessoa\nketik ${prefix}artinama namakamu`, id)
        ...
        // Search Any
	case 'dewabatch':
		if (args.length == 0) return aruga.reply(from, `${prefix}dewabatch judul\n\nContoh: ${prefix}dewabatch naruto`, id)
		...
		break
        case 'images':
            if (args.length == 0) return aruga.reply(from, `em processo\nketik: ${prefix}images [search]\ncontoh: ${prefix}images naruto`, id)
            ...
            break
        case 'sreddit':
            if (args.length == 0) return aruga.reply(from, `para procurar imagens do sub reddit\nketik: ${prefix}sreddit [search]\ncontoh: ${prefix}sreddit naruto`, id)
        ...
        case 'nekopoi':
            ...
            break
        case 'stalkig':
            if (args.length == 0) return aruga.reply(from, `Para perseguir a conta do Instagram de alguém\nketik ${prefix}stalkig [username]\ncontoh: ${prefix}stalkig ini.arga`, id)
        ...
        case 'lyrics':
        case 'lirik':
            if (args.length == 0) return aruga.reply(from, `Para encontrar a letra de uma música\bketik: ${prefix}lirik [judul_lagu]`, id)
            ...
            break
        case 'chord':
            if (args.length == 0) return aruga.reply(from, `Para encontrar as letras e acordes de uma música\bketik: ${prefix}chord [judul_lagu]`, id)
        ...
        case 'ss': //jika error silahkan buka file di folder settings/api.json dan ubah apiSS 'API-KEY' yang kalian dapat dari website https://apiflash.com/
            if (args.length == 0) return aruga.reply(from, `Faça uma captura de tela do bot de uma web\n\nPemakaian: ${prefix}ss [url]\n\ncontoh: ${prefix}ss http://google.com`, id)
        ...
        case 'whatanime':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                aruga.reply(from, 'pesquisando...', id)
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		aruga.reply(from, 'Desculpe, não sei o que é este anime, certifique-se de que a imagem que estará na Pesquisa não está desfocada / cortada', id)
                	}
                    const { is_adult, title, title_chinese, title_romaji, title_english, episode, similarity, filename, at, tokenthumb, anilist_id } = resolt.docs[0]
                    teks = ''
                    if (similarity < 0.92) {
                    	teks = '*Eu tenho pouca confiança nisso* :\n\n'
                    }
				...
				aruga.reply(from, `Desculpe, o formato está incorreto\n\nSilahkan kirim foto dengan caption ${prefix}whatanime\n\nAtau reply foto dengan caption ${prefix}whatanime`, id)
			}
            break
            
        // Other Command
        case 'resi':
            if (args.length !== 2) return aruga.reply(from, `Desculpe, o formato da mensagem está incorreto.\nPor favor, digite a mensagem com ${prefix}resi <kurir> <no_resi>\n\nKurir yang tersedia:\njne, pos, tiki, wahana, jnt, rpx, sap, sicepat, pcp, jet, dse, first, ninja, lion, idl, rex`, id)
            const kurirs = ['jne', 'pos', 'tiki', 'wahana', 'jnt', 'rpx', 'sap', 'sicepat', 'pcp', 'jet', 'dse', 'first', 'ninja', 'lion', 'idl', 'rex']
            if (!kurirs.includes(args[0])) return aruga.sendText(from, `Este tipo de expedição de frete não é compatível com este serviço, apenas oferece suporte a expedições de frete ${kurirs.join(', ')} Tolong periksa kembali.`)
            console.log('Memeriksa No Resi', args[1], 'por expedição', args[0])
        ...
        case 'tts':
            ...
            const dataText = body.slice(8)
                if (dataText === '') return aruga.reply(from, 'qual é o texto syg..', id)
                try {
            ...
            break
        case 'translate':
            if (args.length != 1) return aruga.reply(from, `Desculpe, o formato da mensagem está incorreto.\nPor favor responda a uma mensagem com uma legenda ${prefix}translate <kode_bahasa>\ncontoh ${prefix}translate id`, id)
            if (!quotedMsg) return aruga.reply(from, `Desculpe, o formato da mensagem está incorreto.\nResponda a uma mensagem com uma legenda ${prefix}translate <kode_bahasa>\ncontoh ${prefix}translate id`, id)
            const quoteText = quotedMsg.type == 'chat' ? quotedMsg.body : quotedMsg.type == 'image' ? quotedMsg.caption : ''
            translate(quoteText, args[0])
                .then((result) => aruga.sendText(from, result))
                .catch(() => aruga.sendText(from, 'Error, Código de idioma incorreto.'))
            break
		case 'covidindo':
			rugaapi.covidindo()
			.then(async (res) => {
        ...
        case 'ceklokasi':
            ...
            const text = `*CEK LOKASI PENYEBARAN COVID-19*\nresultado da inspeção do local que você enviou é *${zoneStatus.status}* ${zoneStatus.optional}\n\nInformasi lokasi terdampak disekitar anda:\n${datax}`
		...
		case 'bapakfont':
			...
			break
        case 'hilihfont':
            if (args.length == 0) return aruga.reply(from, `Transformar uma frase em hilih é assim\n\nketik ${prefix}hilihfont kalimat`, id)
		...
		//Fun Menu
    case 'tod':
    aruga.reply(from, 'Antes de jogar, prometa cumprir um determinado comando.\n\nSilahkan Pilih:\n➥ #truth\n➥ #dare', id)
    break
    case 'truth':
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
            ...
            .catch(() => {
                aruga.reply(from, 'Hayolohhh, houve uma falha!!', id)
            })
            break
    case 'dare':
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
        ...
        case 'klasemen':
		case 'klasmen':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, este comando só pode ser usado em grupos!', id)
			const klasemen = db.get('group').filter({id: groupId}).map('members').value()[0]
            let urut = Object.entries(klasemen).map(([key, val]) => ({id: key, ...val})).sort((a, b) => b.denda - a.denda);
            let textKlas = "*Classificações finas temporárias*\n"
            let i = 1;
            urut.forEach((klsmn) => {
	    ...
	    case 'add':
            if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado em grupos!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione bot como administrador do grupo!', id)
        ...
        case 'kick':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione bot como administrador do grupo!', id)
            if (mentionedJidList.length === 0) return aruga.reply(from, 'Desculpe, o formato da mensagem está incorreto.\nMarque uma ou mais pessoas a serem removidas', id)
        ...
        case 'promote':
            ...
            if (groupAdmins.includes(mentionedJidList[0])) return await aruga.reply(from, 'Desculpe, esse usuário já é um administrador.', id)
        ...
        case 'demote':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione bot como administrador do grupo!', id)
            if (mentionedJidList.length !== 1) return aruga.reply(from, 'Desculpe, só pode remover 1 usuário', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return await aruga.reply(from, 'Maaf, user tersebut belum menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Desculpe, o formato da mensagem está incorreto.\nNão é possível rebaixar a própria conta de bot', id)
        ...
        case 'del':
            ...
            if (!quotedMsg) return aruga.reply(from, `Desculpe, o formato da mensagem está incorreto..\nReplique a mensagem do bot com a legenda ${prefix}del`, id)
            if (!quotedMsgObj.fromMe) return aruga.reply(from, `Desculpe, o formato da mensagem está incorreto..\nReplique a mensagem do bot com a legenda ${prefix}del`, id)
		...
		case 'simi':
			...
			if (args[0] == 'on') {
                ...
                aruga.reply(from, 'Ative o bot simi-simi!', id)
			} else if (args[0] == 'off') {
				let inxx = simi.indexOf(chatId)
				simi.splice(inxx, 1)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
				aruga.reply(from, 'Desativa o bot remoto!', id)
			} else {
				aruga.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			}
			break
		...
		case 'kasar':
			if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado em grupos!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
			if (args.length !== 1) return aruga.reply(from, `Para habilitar os recursos de vocabulário no Chat de Grupo\n\nQuais são os usos desse recurso? Quando alguém proferir uma palavrao, receberá uma multa\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			if (args[0] == 'on') {
				ngegas.push(chatId)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				aruga.reply(from, 'O recurso anti-áspero está ativado', id)
			} else if (args[0] == 'off') {
				let nixx = ngegas.indexOf(chatId)
				ngegas.splice(nixx, 1)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				aruga.reply(from, 'O recurso anti-áspero foi desativado', id)
			} else {
				aruga.reply(from, `Para habilitar os recursos de vocabulário no Chat de Grupo\n\nQuais são os usos desse recurso? Quando alguém proferir uma palavrao, receberá uma multa\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			}
			break
		case 'reset':
			if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado em grupos!', id)
		...
		case 'mutegrup':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione bot como administrador do grupo!', id)
			if (args.length !== 1) return aruga.reply(from, `Para alterar as configurações do grupo de bate-papo para que apenas administradores possam bater papo'\n\nPenggunaan:\n${prefix}mutegrup on --aktifkan\n${prefix}mutegrup off --nonaktifkan`, id)
            if (args[0] == 'on') {
				...
				aruga.reply(from, `Para alterar as configurações do grupo de bate-papo para que apenas administradores possam bater papo\n\nPenggunaan:\n${prefix}mutegrup on --aktifkan\n${prefix}mutegrup off --nonaktifkan`, id)
			}
			break
		case 'setprofile':
			if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado em grupos!', id)
			...
			if (isMedia && type == 'image' || isQuotedImage) {
				...
				? aruga.reply(from, 'Desculpe, o link que você enviou não contém imagens.', id)
				: aruga.reply(from, 'Grupo de perfil alterado com sucesso', id))
			} else {
				aruga.reply(from, `Esses comandos são usados ​​para substituir icon/profile group chat\n\n\nPenggunaan:\n1. Silahkan kirim/reply sebuah gambar dengan caption ${prefix}setprofile\n\n2. Silahkan ketik ${prefix}setprofile linkImage`)
			}
			break
		case 'welcome':
			if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado em grupos!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado pelo administrador do grupo!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione bot como administrador do grupo!', id)
			if (args.length !== 1) return aruga.reply(from, `Faça o BOT cumprimentar os novos membros para entrar no chat em grupo!\n\nPenggunaan:\n${prefix}welcome on --aktifkan\n${prefix}welcome off --nonaktifkan`, id)
			if (args[0] == 'on') {
				welcome.push(chatId)
				fs.writeFileSync('./settings/welcome.json', JSON.stringify(welcome))
				aruga.reply(from, 'A mensagem de boas-vindas agora está habilitada!', id)
			} else if (args[0] == 'off') {
        ...
        if (!isOwner) return aruga.reply(from, 'Maaf, este comando só pode ser usado por proprietários de grupos!', id)
        if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione bot como administrador do grupo!', id)
            ...
            aruga.reply(from, 'membro banido com sucesso', id)
        break
        ...
        case 'ban':
            ...
            if (args[0] == 'del') {
                ...
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'alvo desbanido!')
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                banned.push(mentionedJidList[i])
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'alvo banido!', id)
                }
            }
            break
        case 'bc': //untuk broadcast atau promosi
            if (!isOwnerBot) return aruga.reply(from, 'Este pedido é apenas para proprietários de barcos!', id)
            if (args.length == 0) return aruga.reply(from, `Para transmitir para todos os chats, :\n${prefix}bc [isi chat]`)
        ...
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            if (!isOwnerBot) return aruga.reply(from, 'Este comando é apenas para proprietários de barcos', id)
            ...
            aruga.reply(from, 'todos sairam do grupo!', id)
            break
        case 'clearall': //menghapus seluruh pesan diakun bot
            ...
            aruga.reply(from, 'chat limpo !', id)
            break
        default:
            break
        }
